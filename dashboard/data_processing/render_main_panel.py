...
def render_main_panel(df: pd.DataFrame):
    """
    Основная панель: График распределения целевого параметра и график зависимости целевого принака от другого 
    ...
    if df is not None and not df.empty:
        ...
        with aim_cols[0]:
            selected_aim_option = st.selectbox("Выберите целевой параметр", selected_sensors, key="aim_sensor_select")
        with aim_cols[1]:
        ...
        with second_cols[0]:
            selected_second_option = st.selectbox("Выберите вторичный параметр", selected_sensors, key="second_sensor_select")
